...
from nxtheme_creator.process_themes import processImages, THISDIR as nxdir
...
def test_me():
	...
	processImages(nxthemebin=None, inputdir=input_dir, outputdir=output_dir + "_native", config=config)

	# # With SWITCH_THEMES_EXE
	processImages(nxthemebin=SWITCH_THEMES_EXE, inputdir=input_dir, outputdir=output_dir, config=config)
	...
	for file_name in native:
		assert to_json(switch_themes_exe, file_name, "info.json") == to_json(native,file_name, "info.json")
		assert to_json(switch_themes_exe, file_name, "layout.json") == to_json(native,file_name, "layout.json")


def read_nxtheme(file:Path) -> dict[str:bytes]:
	data = file.read_bytes()
	# if oead.yaz0.is_compressed(data):
